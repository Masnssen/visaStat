    ...
    result.forEach(async function(element){
        ...
        if(element.dateF == ""){
            ...
            if(ver == false){
                ...
                console.log(ver);
                console.log("date fin existe pas et ");
            }    
        }else{
            ...
            if(ver){
                ...
                console.log(ver);
                console.log("Mise ajours ", element.num);
            }else{
                //sauvegarder avec date fin
                let test = await inserer(element,2);
                console.log(ver);
                console.log("date fin existe new");
            }
        }
    });
});
...
app.get("/data", function(req, res){ 
   ...
   // let sql = "SELECT num FROM dossier where dateDep = '2021-08-24'"
    db.query(sql, ["2021-08-26", "2021-08-26"], function(err, result, fields){
        if(err) throw err;
...
app.get("/statMois/:date", function(req,res){    
    ...
    if(inf.m < 13 && inf.m > 0 && inf.type > 0 && inf.type < 4){
