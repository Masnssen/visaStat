...
///La variable qui a la liste des dossiers a verifier si le traitement est terminer
var dossierVerif;
		...
		setTimeout(function(){
			if(load.style.display == "block"){
				...
				if(existePas.style.display == "none"){
                    nbInconnue = 0; ///Remetre le compteur des dossier succesive inconnue
					let enr = {
							num : "",
							dateI : "",
							dateF : "",
					...
					nbInconnue++; //Incrimenter le compteur des dossier successive inconnue
				}
				resolve(true);
			}
		...
		setTimeout(function(){
			if(load.style.display == "block"){
				...
				if(existePas.style.display == "none"){
					...
					enr.dateF = "01-01-2000";
					enr.v = "inconnu";
					enr.num = numD.value;
					enr.dateI = "01-01-2000";
					tab.push(enr);
				}
...
///Fonction qui recupere les dossiers a verifier si il sont en fin de traitement
function recupererDossier(){
	var xhr = new XMLHttpRequest();
	xhr.open("GET", 'http://localhost:8080/data', true);
	...
	xhr.send(body);
	xhr.onload = function(){
		dossierVerif = JSON.parse(this.response);
  		console.log(dossierVerif);
	}
}
